...
export default function App() {
  ...
  function uploadDeposit(id, file) {
    const fields = new FormData();
    fields.append('file', file);
    return change(`/api/accounts/${id}/deposits/upload`, 'POST', fields,
      'Deposit imported from text file.');
  }
      ...
      {!user ? <Welcome bankInfo={bankInfo} onSubmit={authenticate} busy={busy} /> : <>
        ...
        {isAdmin ? <AdminAccounts accounts={adminAccounts} onStatus={changeStatus} busy={busy} /> :
          <CustomerDashboard accounts={accounts} selectedId={selectedId} transactions={transactions}
            onSelect={chooseAccount} onMoveMoney={moveMoney} onTransfer={transfer}
            onUploadDeposit={uploadDeposit} busy={busy} />}
      </>}
    </main>
